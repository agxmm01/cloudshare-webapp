    ...
    useEffect(() => {
        const layout = () => {
            circleRefs.current.forEach((circle) => {
                ...
                if (label) tl.to(label, { opacity: 0, duration: 2, ease, overwrite: 'auto' }, 0);
                if (white) {
                    gsap.set(white, { y: 0, opacity: 0 });
                ...
                <div className="pill-nav-items desktop-only" ref={navItemsRef}>
                    <ul className="pill-list" role="menubar">
                        {items.map((item, i) => {
                            ...
                            const pillClass = `pill${isActive ? ' is-active' : ''}${item.cta ? ' pill-cta' : ''}`;
                            return (
                                <li key={item.href || `item-${i}`} role="none">
                                    {isRouterLink(item.href) ? (
                                        <Link role="menuitem" to={item.href}
                                            ...
                                            onMouseEnter={() => !item.cta && handleEnter(i)}
                                            onMouseLeave={() => !item.cta && handleLeave(i)}>
